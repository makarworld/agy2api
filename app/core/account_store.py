    ...
    with get_db_connection() as conn:
        for item in acc_list:
            ...
            refresh_token = item.get("refresh_token") or (
                item.get("credentials", {}).get("refresh_token") if isinstance(item.get("credentials"), dict) else None
            )
            ...
            client_secret = item.get("client_secret") or (
                item.get("credentials", {}).get("client_secret") if isinstance(item.get("credentials"), dict) else ""
            )
            access_token = item.get("access_token") or (
                item.get("credentials", {}).get("access_token") if isinstance(item.get("credentials"), dict) else ""
            )
            project_id = item.get("project_id") or ""

            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, access_token, token_expiry, status, backoff_until FROM accounts WHERE id = ?", (acc_id,)
            )
...
def find_account_by_refresh_token(
    refresh_token: str, exclude_account_id: Optional[str] = None
    ...
    init_accounts_table()
    with get_db_connection() as conn:
        query = "SELECT * FROM accounts WHERE refresh_token = ?"
...
def update_account_tokens(
    account_id: str, access_token: str, token_expiry: float, project_id: Optional[str] = None
) -> None:
    now = time.time()
    with get_db_connection() as conn:
        if project_id:
...
def upsert_account(
    *,
    ...
    if duplicate:
        raise ValueError(
            f"Refresh token already assigned to account {duplicate['id']}; "
            "complete OAuth login for the intended Google account"
        )
    init_accounts_table()
    now = time.time()
    with get_db_connection() as conn:
