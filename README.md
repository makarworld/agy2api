...
- **Менеджер API-ключей и разграничение доступа:**
  - Поддержка создания ограниченных (Limited) API-ключей (`sk-agy-...`) с лимитами по дневным выходным токенам (Output Tokens) и сроку действия (TTL).
  - Встроенный механизм скользящего окна ограничения запросов: **Rate Limiting (RPM/TPM)** по каждому ключу.
  - Возможность мгновенной блокировки/деактивации ключей через вкладку **API Keys** веб-интерфейса.
- **Интеллектуальная маршрутизация и устойчивость пула:**
  - **Per-Model Cooldown:** При получении 429 ошибки на конкретную модель (например, `claude-3-5-sonnet`) аккаунт блокируется в пуле только для этой модели, оставаясь доступным для остальных (`gemini`, `flash` и т.д.).
  - **Session Affinity (липкие сессии):** Запросы в рамках одного контекста/диалога направляются на один и тот же Google-аккаунт, минимизируя сбросы кэша и ошибки ротации.
  - Поддержка индивидуальных HTTP/SOCKS5 прокси для каждого Google-аккаунта с автоматическим fallback на общий `AGY_GOOGLE_PROXY`.
- **Оптимизация базы данных и автоочистка:**
  - Автоматическая очистка тяжелых текстов промптов и ответов в SQLite старше 30 дней с сохранением всех метаданных статистики и счетчиков токенов.
- **Новый интерфейс и руководство по интеграции:**
  - Интерактивный мастер подключения (Integration Guide) прямо на странице управления API-ключами.
  - Детальные фильтры запросов, визуализация статусов моделей и аккаунтов.
...
Самый простой и быстрый способ начать работу на Windows без настройки окружения разработчика:

1. **Скачивание:**
   - Перейдите в раздел [Releases](https://github.com/makarworld/agy2api/releases) и скачайте архив релиза **`agy2api-windows-x64.zip`** (или папку с `agy2api.exe`).
2. **Распаковка:**
   - Распакуйте архив в любую постоянную папку (например, `C:\agy2api` или `D:\Tools\agy2api`).
   - *Важно:* Убедитесь, что папка `_internal` и файл `agy2api.exe` находятся в одной директории.
3. **Настройка окружения (`.env`):**
   - Скопируйте файл `.env.example` и переименуйте его в `.env` (он должен лежать в той же папке рядом с `agy2api.exe`).
   - Откройте `.env` в блокноте или любом редакторе и задайте параметры:
     - `ADMIN_PASSWORD` — пароль для входа в веб-дашборд.
     - `AGY_API_KEY` и `ANTHROPIC_COMPAT_API_KEY` — мастер-ключи доступа к серверу.
     - `PORT` и `HOST` — по умолчанию `8000` и `0.0.0.0` (можно сменить при необходимости).
     - Если Google заблокирован в вашем регионе — настройте `AGY_GOOGLE_PROXY=http://user:pass@ip:port`.
4. **Запуск бинарника:**
   - Запустите **`agy2api.exe`** двойным кликом (или через командную строку `agy2api.exe`).
   - В открывшемся окне консоли появится лог запуска сервера: `[*] Starting AGY2API on http://0.0.0.0:8000`.
5. **Вход в панель управления:**
   - Откройте в браузере адрес: **`http://localhost:8000`** (или указанный вами порт).
   - Введите пароль `ADMIN_PASSWORD` из вашего `.env`.
6. **Подключение первого аккаунта:**
   - Перейдите в раздел **Account Pool**, нажмите **Generate Login Link** и привяжите аккаунт через Google OAuth в 1 клик.
