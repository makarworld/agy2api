  ...
  const CopyBtn = ({ text, label }: { text: string; label?: string }) => {
    ...
    return (
      <button
        ...
        className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors px-2 py-1 rounded bg-muted/70 hover:bg-muted font-sans"
      >
        {copied ? <Check className="w-3 h-3 text-emerald-500" /> : <Copy className="w-3 h-3 text-muted-foreground" />}
        <span>{copied ? 'Скопировано' : label || 'Copy'}</span>
      </button>
    );
  };

  return (
    <div className="border rounded-xl bg-card text-card-foreground shadow-sm overflow-hidden border-primary/20">
      <div
        onClick={() => setOpen(!open)}
        className="px-4 py-3 bg-muted/40 hover:bg-muted/60 transition-colors flex items-center justify-between cursor-pointer select-none"
      >
      ...
      {open && (
        <div className="p-5 space-y-4 text-xs sm:text-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* OpenAI Standard */}
            <div className="p-4 rounded-lg border bg-background/80 space-y-2.5">
